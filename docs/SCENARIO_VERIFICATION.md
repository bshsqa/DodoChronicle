...
**시나리오:** 이벤트 카드를 탭하면 이벤트 상세 화면으로 이동한다.

✅ **PASS**

- `onClick = { onEventClick(event.id) }` → NavController로 `Screen.EventDetail` 이동
- 요구사항 UI-06은 v1.3에서 "상세 화면 이동"으로 수정됨 (기존 "확장 애니메이션" 표현 제거)
...
**시나리오:** 타임라인 상단 메뉴에서 "초기화 재실행"을 선택한다.

⏳ **미구현 (Phase 4)**

- 설정 화면(`Screen.Settings`) 자체가 없음
- `Screen.kt`에 `Init`, `Timeline`, `EventDetail` 3개만 존재
- REQUIREMENTS.md §3.7 SET-01 참고

---

### 2-14. 설정 화면 — 데이터 전체 초기화

**시나리오:** 설정 화면에서 "데이터 전체 초기화"를 선택하고 확인한다.

⏳ **미구현 (Phase 4)**

- 설정 화면 미구현으로 접근 불가
- REQUIREMENTS.md §3.7 SET-02 참고

---

### 2-15. 사진 권한 거부

**시나리오:** 앱 설치 후 사진 접근 권한을 거부한다.

⏳ **미구현 (Phase 4)**

- 현재 `MainActivity`에서 권한 요청 UI 존재하나, 거부 시 별도 안내 없이 스캔 시 0장 처리됨
- REQUIREMENTS.md PERM-03 참고
...
> ❌ 5건은 모두 코드 수정 완료 (B-01, B-02, B-03).
> ⏳ 3건(설정 화면 재실행, 전체 초기화, 권한 거부 처리)은 Phase 4 구현 예정.
