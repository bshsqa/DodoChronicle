...
## 목표 (Objective)
앱에 이미 생성된 이벤트를 하나의 파일로 내보내고, 다른 사용자가 해당 파일의 다운로드 가능한 링크를 입력하면 같은 이벤트를 자신의 앱으로 가져올 수 있게 합니다.
...
4. 사용자 B가 앱의 `이벤트 가져오기`에 링크를 입력합니다.
5. 앱이 파일을 다운로드하고 텍스트 이벤트를 import합니다.
...
`이벤트 가져오기`를 누르면 URL 입력 다이얼로그를 표시합니다.
